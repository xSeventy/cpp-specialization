...
public:
  ...
  void SetLogFile(bool value) { log_file = value; }
    ...
    bool Get_log_line () { return log_line; }
    bool Get_log_file () { return log_file; }
...
#define LOG(logger, message) {                                  \
    stringstream os;                                            \
    if(!logger.Get_log_line() && !logger.Get_log_file()) {      \
                                                                \
        os << message;                                          \
                                                                \
        ...
        os << "Line " << __LINE__ << " "                        \
           << message;                                          \
                                                                ...
                                                                \
        os << __FILE_NAME__ << " " << message;                  \
                                                                \
    } else {                                                    \
                                                                \
        os << __FILE_NAME__ << ":" << __LINE__                  \
           << " " << message;                                   \
    }                                                           \
    logger.Log(os.str());                                       \
}
